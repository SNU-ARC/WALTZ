...
class DBImpl : public DB {
 ...
 protected:
  ...
  mutable InstrumentedMutex deadlock_protector_mutex_;
 ...
 private:
  ...
  inline void WaitForPendingWrites() {
    ...
    // [JS] we should wait for pending_memtable_writes flag, so disable the return code  -- only for waltz_mode case
    if (!waltz_mode) {
  ...
  IOStatus ZnsAppendToWAL(const WriteBatch& merged_batch, log::Writer* log_writer,
